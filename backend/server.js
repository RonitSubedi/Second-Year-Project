...
io.on("connection", (socket) => {
  ...
  socket.on("driver:identify", ({ token }) => {
    try {
      ...
      console.log(
        `🚌 Driver identified: ${decoded.name} → Route: ${decoded.assignedRoute || "ALL (admin)"}`,
      );
    } catch {
  ...
  socket.on("driver:update_location", (data) => {
    ...
    if (
      socket.driverRole === "driver" &&
      allowedRoute &&
      data.route !== allowedRoute
    ) {
    ...
    console.log(
      `📍 ${routeKey} bus → ${data.lat.toFixed(5)}, ${data.lng.toFixed(5)}`,
    );
  });
